package me.shini9000.abmcjudge.SQL;
...
public class SQLCreate {
    ...
    public void createPlot(String id, Player player, String title, String lore, String status, String comment) {
        try {
            PreparedStatement ps = this.plugin.SQL.getConnection().prepareStatement("SELECT * FROM plotsubmit WHERE PLOTID=?");
            ps.setString(1, id);
            ResultSet results = ps.executeQuery();
            results.next();
            UUID uuid = UUID.nameUUIDFromBytes(id.getBytes());
            if (!exists(uuid)) {
                PreparedStatement ps2 = this.plugin.SQL.getConnection().prepareStatement("INSERT IGNORE INTO plotsubmit (NAME,UUID,PLOTID,PLOTTITLE,PLOTLORE,STATUS,COMMENT) VALUES (?,?,?,?,?,?,?)");
    ...
    public boolean exists(UUID uuid) {
        try {
            PreparedStatement ps = this.plugin.SQL.getConnection().prepareStatement("SELECT * FROM plotsubmit WHERE UUID=?");
